...
--
-- S10: el corpus está particionado en 3 colecciones (budget_chunks, transcript_chunks,
-- technical_doc_chunks). Estas queries apuntan a budget_chunks; repítelas cambiando el
-- nombre de tabla/índice para auditar las otras dos colecciones (mismo esquema e índices).
...
--   CREATE INDEX CONCURRENTLY budget_chunks_embedding_halfvec_idx
--     ON budget_chunks USING hnsw ((embedding::halfvec(1536)) halfvec_cosine_ops)
