"""Endpoint de ingesta de embeddings. Orquesta chunker → embedder → IngestResponse.
def síncrono a propósito (FastAPI lo corre en threadpool): litellm.embedding es
bloqueante; no queremos bloquear el event loop.
"""
...
def ingest(request: IngestRequest) -> IngestResponse:
    ...
    try:
        ...
        logger.exception("embeddings.ingest_failed", budgets=len(request.budgets))
    ...
    logger.info("embeddings.ingest_done", **stats.model_dump())
